...
module Papyrus
  # The Tokenizer is responsible for splitting and categorizing the content of
  ...
  # content categorized, we can then loop through and interpret it later.
  #
  # Tokenizers are really only used internally -- you shouldn't have to create
  # one yourself unless you're doing something specific.
  #
  class Tokenizer
    extend Forwardable

    # Create a new Tokenizer, storing the Parser this Tokenizer belongs to.
    #
    ...
    delegate [:content] => :@template

    # Step through the content and split it into an array of tokens. Possible
    # tokens are:
    #
    # * Token::LeftBracket
    # * Token::RightBracket
    ...
    #
    # * Quotes, brackets, and forward slashes are represented by their own
    #   tokens and those tokens are pushed individually onto the token list.
    # * Words and anything else that's not a quote, bracket or slash character
    #   are grouped together in a Token::Text.
    # * If a left or right bracket is preceded by a backslash, then we use a
    #   Token::Text to represent the bracket instead of a Token::LeftBracket or
    #   Token::RightBracket, so that the sub isn't interpreted.
    # * The set of tokens that represent a sub is always wrapped in a TokenList
    #   so we can easily detect it later.
    #
    # At the end of the tokenization, @tokens will hold the token list.
    #
    # NOTE: There's a tiny bit of magic going on when we actually go to add the
    # token to the token list. See TokenList#add for what's up.
    #
    def tokenize
      @stack = [ TokenList.new ]
      @num_open_brackets = 0
    ...
    def _build_token_list_inside_sub
      if str = content.scan(/\]/)
        _flush_token_buffer
    ...
    def _build_token_list_outside_sub
      if content.scan(/\[/)
        @num_open_brackets += 1
        _flush_token_buffer
        @stack << TokenList[ Token::LeftBracket.new ]
