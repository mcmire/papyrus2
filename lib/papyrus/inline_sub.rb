...
module Papyrus
  class InlineSub < Sub
    ...
    # Resolve the sub and return the result. This is where we determine what
    # exactly the Sub actually represents: whether it's a built-in command, a
    # custom command, a variable - or whether it in fact doesn't resolve to
    # anything.
    #
    ...
    # be resolved, the result of this sub will still be retained. This works
    # because there's actually a reference to this raw tokens object in the raw
    # tokens array of the parent sub, and so changing this object changes that
    # one (and any other references up the ancestry chain) too. Neat, eh?
    #
    def evaluate
    ...
    def _evaluates_as_builtin_command?
      # this isn't strictly inline, but right now we only have one built-in
      # command and it's inline... in short, FIXME later
