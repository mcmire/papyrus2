...
module Papyrus
  ...
  #
  class Text < Node
    # Create a new Text node, storing the given text.
    #
    def initialize(text)
      @text = text.to_s
    end
    ...
    # Just return the text given to Text.new.
    #
    def evaluate
      # make a new object here so that when we build strings in
      # NodeList#evaluate we don't end up passing references around and
      # destructively modifying strings that originated all the way over here
      @text.dup
    end
