...
module Papyrus
  # A Template is the grand central station of the parsing process. It
  ...
  class Template
    ...
    # Parses the substitutions in the source string and replaces them with
    # their corresponding values. The replacement text of [body] subs are
    # specially processed if so specified. Returns the final string.
    #
    def self.render(content, options = {})
      new(content, options).render
    end

    # Creates a new Template, storing the given content and options. Content is
    # usually a string, but may also be an instance of StringScanner. Available
    # options are:
    #
    # :vars                 - Template variables which will be set in the
    ...
    # :shielded_commands    - An array of command names that will be "shielded"
    #                         -- i.e., if there are any subs passed as arguments
    ...
    #                         to your CustomCommandSet.
    #
    # All options above will also be used by the Parser and Tokenizer, created
    ...
    attr_reader :shielded_commands
    attr_accessor :parent
    attr_reader :allowed_commands

    delegate [:[], :[]=] => :@vars

    # Run the content through the tokenizer to produce an array of tokens, then
    ...
    # Parse the substitutions in the source string and replace them with their
    ...
    # Ensure that the given content is stored as a StringScanner, which provides
    ...
    # Clone the current template, allowing you to specify new content, options,
